...
public class LGDecription {
	...
	public void sendDescription() {
		...
		if (role == LGRoles.Mineur) {
			...
			String role_Use = "§9Pour cela vous pourrez vous stuff plus facilement grâce à un effet permanent de Haste II ! ";
		...
		if (role == LGRoles.Chasseur) {
			...
			String role_Use = "§9Pour ce faire, a votre mort vous pouvez choisir un joueur qui perdra la moitié de sa vie grâce à la commande"
					+ " /lg pan <Pseudo>";
		...
		if (role == LGRoles.Salvateur) {
			...
			String role_Use = "§9Pour y arriver, à chaque début d'épisode, vous pouvez choisir un joueur (y compris vous même) a qui vous conférez "
					+ "NoFall et résistance pendant 20min, à l'aide la commande /lg protect. Vous ne pouvez pas choisir 2 d'afilée la même personne !";
